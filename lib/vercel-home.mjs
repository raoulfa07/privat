...
async function notifyHouseholdItem(item) {
  const apiKey = process.env.RESEND_API_KEY;
  const to = process.env.HOUSEHOLD_NOTIFICATION_TO;
  if (!apiKey || !to) return;

  const from = process.env.HOUSEHOLD_NOTIFICATION_FROM
    || "Ich mag Rippchen <onboarding@resend.dev>";
  const photoLink = item.photo
    ? `<p><a href="${escapeHtml(item.photo)}">Foto ansehen</a></p>`
    : "";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      "Idempotency-Key": `household-${item.id}`,
    },
    body: JSON.stringify({
      from,
      to: [to],
      subject: `Neues Fundstück: ${item.task}`,
      html: `
        <h2>Charleen hat ein Fundstück gemeldet</h2>
        <p><strong>Was:</strong> ${escapeHtml(item.task)}</p>
        <p><strong>Wo:</strong> ${escapeHtml(item.room)}</p>
        <p><strong>Dringlichkeit:</strong> ${escapeHtml(item.tone)}</p>
        ${photoLink}
        <p><a href="https://www.ich-mag-rippchen.de/#home">Zur Pinnwand</a></p>
      `,
      text: [
        "Charleen hat ein Fundstück gemeldet.",
        `Was: ${item.task}`,
        `Wo: ${item.room}`,
        `Dringlichkeit: ${item.tone}`,
        item.photo ? `Foto: ${item.photo}` : "",
        "Pinnwand: https://www.ich-mag-rippchen.de/#home",
      ].filter(Boolean).join("\n"),
    }),
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`Resend antwortete mit ${response.status}: ${error.slice(0, 300)}`);
  }

  const result = await response.json();
  console.info(`Fundstück-Mail an ${to} übergeben: ${result.id || "ohne Resend-ID"}`);
  ...
  if (result.id) {
    ...
    if (statusResponse.ok) {
      ...
      console.info(`Status der Fundstück-Mail ${result.id}: ${status.last_event || "unbekannt"}`);
    }
  }
}
...
async function createHousehold(request) {
  ...
  try {
    await notifyHouseholdItem(item);
  } catch (error) {
    console.error("Fundstück gespeichert, Mailversand fehlgeschlagen:", error);
  }
  return json(item, 201);
}
